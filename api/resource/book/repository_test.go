...
func TestRepository_Create(t *testing.T) {
	...
	book := &book.Book{ID: id, Title: "Title", Author: "Author", PublishedDate: time.Time{}}
	_, err = repo.Create(book)
	testUtil.NoError(t, err)
}

func TestRepositroy_Read(t *testing.T) {
	t.Parallel()
...
func TestRepository_Update(t *testing.T) {
	...
	mock.ExpectExec("^UPDATE \"books\" SET").
		WithArgs("Title", "Author", mockDB.AnyTime{}, "", "", mockDB.AnyTime{}, mockDB.AnyTime{}, id).
		WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectCommit()
...
func TestRepository_Delete(t *testing.T) {
	...
	id := uuid.New()
	_ = sqlmock.NewRows([]string{"id", "title", "string"}).
		AddRow(id, "Book1", "Author1")
